...
@interface YBHelpWithMeVC ()<YBOwnersHelpAlertViewDelegate>
@property(nonatomic,strong) NSMutableArray  *dataArr;
@end

@implementation YBHelpWithMeVC
...
-(void)requstData{
    ...
    parm[@"touserid"] = userID;
     YBLog(@"parm==%@",parm);
    WEAK_SELF;
    [YBRequest postWithURL:DriverhelpDriverhelpinfolistbytouserid MutableDict:parm success:^(id dataArray) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([state isEqualToString:@"拒绝帮助"]) {
        
    }else if ([state isEqualToString:@"未处理"]){
        YBOwnersHelpAlertView  *helpView = [[YBOwnersHelpAlertView alloc] init];
        helpView.VC = self;
        helpView.helpButton.hidden = YES;
        helpView.nohelpButton.hidden = NO;
        helpView.tohelpButton.hidden = NO;
