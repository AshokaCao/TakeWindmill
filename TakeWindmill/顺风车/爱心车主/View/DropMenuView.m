...
@interface DropMenuView ()<UITableViewDelegate, UITableViewDataSource,UIGestureRecognizerDelegate>
{
@private
    /** 保存 选择的数据(行数) */
    NSInteger selects[3];
...
-(void)creatDropViewY:(CGFloat)viewY withShowTableNum:(NSInteger)tableNum withData:(NSArray *)arr
{
    ...
    if (!self.show) {
        ...
        CGFloat y = 0.f;
        CGFloat w = YBWidth;
        CGFloat h = YBHeight + 64;
        
        self.frame = CGRectMake(x, y, w, h);
        self.cancelButton.frame = CGRectMake(0, viewY,self.frame.size.width, YBHeight - viewY + 64);
...
-(void)adjustTableViews{
    ...
    for (int i = 0; i < addTableCount; i++) {
        ...
        adjustFrame.size.width = YBWidth / addTableCount ;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    ...
    if ([touch.view isKindOfClass:[UIButton class]] || [touch.view isDescendantOfView:self.tableViewUnderView]) {
        //放过button点击拦截
        return NO;
    }
    return YES;
